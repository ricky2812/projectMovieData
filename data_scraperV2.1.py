...
def work(l_id):
    def web(id):
        ...
        try:
            tester = WebDriverWait(driver, 10).until(
                EC.presence_of_element_located((By.XPATH, '//*[@class="sc-b73cd867-0 eKrKux"]'))
            ).text
        finally:
            tester = 'N/A'
        try:
        ...
        ac = driver.find_elements_by_xpath('//*[@class="sc-18baf029-1 gJhRzH"]')
        if len(ac) > 0:
            actors = []
            for i in ac:
